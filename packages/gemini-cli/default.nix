...
buildNpmPackage (finalAttrs: {
  ...
  passthru.updateScript = lib.getExe (writeShellApplication {
    ...
    text = ''
      ...
      sed -i "s|version = ".*";|version = "$latest_version";|" "packages/gemini-cli/default.nix"
      sed -i "s|rev = ".*";|rev = "$latest_rev";|" "packages/gemini-cli/default.nix"
      sed -i "/src = fetchFromGitHub/,/};/s|hash = ".*";|hash = "$src_hash";|" "packages/gemini-cli/default.nix"
      sed -i "/npmDeps = fetchNpmDeps/,/};/s|hash = ".*";|hash = "$npm_deps_hash";|" "packages/gemini-cli/default.nix"
    '';
  });
