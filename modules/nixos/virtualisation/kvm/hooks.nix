...
let
  ...
  qemu-hooks = config.${namespace}.virtualisation.kvm.hooksPackage;
  ...
  #   ${optionalString (config.hardware.nvidia.nvidiaPersistenced) "systemctl stop nvidia-persistenced.service 2>/dev/null || true"}
  #   ${optionalString (config.spirenix.services.ollama.enable) "systemctl stop ollama.service 2>/dev/null || true"}
  #
  #   # Kill dGPU processes
...
in
{
  config = mkIf (cfg.enable && cfg.vfio.enable) {
    ...
    systemd.services.libvirtd.path = [ pkgs.bash qemu-hooks ];
    ...
    virtualisation.libvirtd.hooks.qemu = {
      "qemu-hook-dispatcher" = "${qemu-hooks}/bin/qemu-hook-dispatcher";
      ...
      #       ${kvm-hooks}/bin/give-vfio-dgpu
      #       ${kvm-hooks}/bin/virsh-check-usb
