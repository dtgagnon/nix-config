...
in
{
  options.${namespace}.apps.antigravity = {
    ...
    extraConfig = mkOpt (types.attrsOf types.anything) { } "Initial configuration for antigravity to use";
  };
  ...
  config = mkIf cfg.enable {
    home.packages = [
      ...
      # Wrap Chrome with flags to prevent Hyprland color management crashes
      (pkgs.google-chrome.override {
        commandLineArgs = [
          "--ozone-platform=wayland"
          "--enable-features=UseOzonePlatform"
          # Disable GPU features that trigger HDR color management checks
    ...
    xdg.configFile = {
      "Antigravity/product.json".text = builtins.toJSON {
        extensionsGallery = {
      ...
      "Antigravity/User/settings.json".text = builtins.toJSON {
        ...
        "antigravity.browserUserProfilePath" = "/home/dtgagnon/.gemini/antigravity-browser-profile";
        ...
        # Nested objects look like this in Nix
        ...
        # --- Rust & JSON Settings ---
        # I have removed the "null" keys here to prevent errors
